...
const HospitalDashboard = () => {
  ...
  const getEstimatedWaitTime = (department: string, priority: "High" | "Medium" | "Low", patients: Patient[]) => {
    // Find all patients in the same department
    const departmentPatients = patients.filter((p: Patient) => p.department === department);
  
    if (departmentPatients.length === 0) {
      // If no previous patients in the department, set a base time
      return priority === "High" ? 5 : priority === "Medium" ? 15 : 25;
    }
  
    // Get the last patient in the department to set a reference
    const lastPatient = departmentPatients[departmentPatients.length - 1];
  
    // Set time increments based on priority
    ...
    // Ensure new patient's time is always greater than the last patient's estimated wait time
    return lastPatient.estimatedWaitTime + priorityAdjustments[priority];
  };
  ...
  const handleAddPatient = () => {
    ...
    const newPatient: Patient = {
      ...
      estimatedWaitTime: getEstimatedWaitTime(selectedDepartment, selectedPriority, patients),
    };
  ...
  const handleStatusChange = (patientId: string, newStatus: 'Waiting' | 'In Progress' | 'Completed') => {
    setPatients(patients.map((patient: Patient) => 
...
//     patientId: string,
//     newStatus: 'Waiting' | 'In Progress' | 'Completed'
//   ) => {
...
//         const completedPatient = prevPatients.find(p => p.id === patientId);
//         if (!completedPatient) return updatedPatients;

//         const { department, estimatedWaitTime: completedWaitTime } = completedPatient;
...
//             return {
//               ...patient,
//               estimatedWaitTime: Math.max(0, patient.estimatedWaitTime - completedWaitTime),
//             };
//           }
//           return patient;
//         });
